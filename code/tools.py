...
def import_data(dataset, undirected=False, ego='source', alter='target', force_dense=True, noselfloop=True, verbose=True, header=0,sep=',',
				binary=False):
	"""
...
def read_graph(df_adj, ego='source', alter='target', undirected=False, noselfloop=True, verbose=True, binary=True):
	...
	# 			else:
	# 				if A[l].has_edge(v1, v2):
	# 					A[l][v1][v2][0]['weight'] += int(row[1 + 2])  # the edge already exists, no parallel edge created
	# 				else:
	# 					A[l].add_edge(v1, v2, weight=int(row[1 + 2]))
	# if verbose:
	# 	print('done!')
