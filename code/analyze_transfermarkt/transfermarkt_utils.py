...
def plotting_map(
	...
	nx.draw_networkx_edges(graph, pos, edgelist=edgelist, width=edge_width, alpha=0.8, node_size=0, ax=ax,
						   edge_color=edge_color, arrows=True, arrowsize=40, style='-', connectionstyle="arc3,rad=0.4")
	nx.draw_networkx(graph, pos, nodelist=nodelist, node_size=250, node_color='grey', edgelist=[], with_labels=False, ax=ax, alpha=0.8)
	
	updated_graph = nx.MultiDiGraph() 
	for edge in graph.edges():
		source, target = edge
		updated_source = source.replace('_', ' ')
		updated_target = target.replace('_', ' ')
		updated_graph.add_edge(updated_source, updated_target)
	# Assuming pos is your dictionary
	updated_pos = {}

	for node, coordinates in pos.items():
		new_name = node.replace('_', ' ')
		updated_pos[new_name] = coordinates 
	
	label_options = {"ec": "grey", "fc": "white", "alpha": 0.5} 
	
	
	labels = nx.draw_networkx_labels(updated_graph, updated_pos, font_size=font_size, alpha=0.75, horizontalalignment='center', verticalalignment='top', ax=ax,font_family='Helvetica', font_weight='bold', bbox=label_options) 
	for node, text in labels.items():
		if node in target_nodes:
			font_properties = FontProperties(family='Helvetica', size = 2.*font_size)  # Customize as needed
			text.set_fontproperties(font_properties)
			text.set_weight('heavy')     
		
		if node == 'St. Petersburg' or  node == 'Cagliari Calcio' or  node == 'Chievo Verona':
			font_properties = FontProperties(family='Helvetica', size = font_size-1.5)  # Customize as needed
			text.set_fontproperties(font_properties)
