...
def import_data(dataset, undirected=False, ego='source', alter='target', force_dense=True, noselfloop=True, verbose=True,header=0,sep='\s+', binary=True): 
	"""
	...
	A = read_graph(df_adj=df_adj, ego=ego, alter=alter, undirected=undirected, noselfloop=noselfloop, verbose=verbose,binary=binary)
...
def plot_M(M, cmap = 'PuBuGn', title=''):
	"""
	...
	ax.set_title(title+' MT means matrix', fontsize = 15)
